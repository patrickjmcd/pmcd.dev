...
import { toExt } from '@/util/helpers';
...
import { RichText, RichTextSegment } from '@/utils/RichText';
...
function LeafletBlock({ block }: { block: linearDocument.Block }) {
  ...
  return (
    <div>
      <strong>TODO: {innerBlock.$type}</strong>
    </div>
  );
}
...
function LeafletBlockHeader({ block }: { block: blocks.header.Main }) {
  ...
  if (level === 2) {
    return (
      <h2>
        <RenderRichText rt={new RichText(block.plaintext, block.facets)} />
      </h2>
    );
  }
  if (level === 3) {
...
function LeafletBlockUl({
  block,
}: {
  block: blocks.unorderedList.Main | blocks.unorderedList.ListItem;
}) {
  return (
    <ul>
...
function LeafletBlockBskyPost({ block }: { block: blocks.bskyPost.Main }) {
  return <BlueskyPostEmbed uri={block.postRef.uri} cid={block.postRef.cid} />;
}

function LeafletBlockCode({ block }: { block: blocks.code.Main }) {
  return (
    <pre>
      <code className={`code-highlight language-${block.language}`}>{block.plaintext}</code>
    </pre>
  );
}
